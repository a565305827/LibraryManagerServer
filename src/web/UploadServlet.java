...
//        upload.setHeaderEncoding("utf-8");
        ...
        try {
            ...
            for (FileItem fileItem:list) {
                if (fileItem.getFieldName().equals("imageurl")) {
                    break;
                }
...
//                System.out.println("*************");
            ...
            for (FileItem fileItem:list){
                if (!fileItem.isFormField()&&fileItem.getName()!=null&&!"".equals(fileItem.getName())){
                    ...
                    fileItem.write(new File(uploadTemp.getAbsolutePath() + File.separator,uuid+suffix));
                    request.setAttribute("imageurl", uuid+suffix);
                }
            }

            request.getRequestDispatcher("/BooksLibrary").forward(request, response);
            //response.sendRedirect("/BooksLibrary");
        }  catch (Exception e) {
            e.printStackTrace();
        }
    }
